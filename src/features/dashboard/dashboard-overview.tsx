    ...
    <div className="space-y-6">
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          label="مواعيد اليوم"
          value={metrics.today}
          hint="مواعيد مسجلة لهذا اليوم"
          icon={CalendarCheck2}
        />
        <MetricCard
          label="هذا الأسبوع"
          value={metrics.thisWeek}
          hint="مواعيد الأيام السبعة القادمة"
          icon={CalendarRange}
        />
        <MetricCard
          label="زبائن جدد"
          value={metrics.newCustomers}
          hint="استفسارات بانتظار المتابعة"
          icon={UserPlus}
        />
        <MetricCard
          label="بحاجة لمتابعة"
          value={metrics.needsFollowUp}
          hint="ملفات دون تحديث حديث"
          icon={CircleAlert}
        />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          {reminders.length === 0 ? (
            <DashboardEmptyState
              ...
              body="ستظهر المواعيد المجدولة للغد هنا."
            />
          ) : (
            <div className="space-y-3">
              {reminders.map((appointment) => (
                <div
                  key={appointment.id}
                  className="rounded-lg bg-violet-50 px-3 py-3 text-sm text-violet-950"
                >
                  <p className="font-medium">{customersById.get(appointment.customerId)?.name}</p>
                  <p className="mt-1 text-xs text-violet-700">
                    {appointment.purpose} — عبر WhatsApp وتنبيه المشرفة
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          {followUps.length === 0 ? (
            <DashboardEmptyState
              title="كل الملفات محدثة"
              body="لا توجد زبائن بحاجة لمتابعة حاليًا."
            />
          ) : (
            <div className="space-y-3">
