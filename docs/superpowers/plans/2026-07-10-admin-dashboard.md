...
const customers: Customer[] = [
  {
    ...
    notes: [],
    activity: [
      { id: "activity-1", label: "تم إنشاء ملف الزبونة", createdAt: "2026-07-05T08:00:00.000Z" },
    ],
  },
  {
    id: "customer-2",
    ...
    notes: [
      {
        id: "note-1",
        body: "تفضّل أقمشة طبيعية بقصّة هادئة.",
...
describe("DashboardOverview", () => {
  it("switches between day and week schedule views", () => {
    render(
      <DashboardOverview {...demoDashboardState} now={new Date("2026-07-10T08:00:00.000Z")} />,
    );
...
```ts
expect(
  filterAppointments(appointments, customers, {
    query: "ليان",
    type: "all",
    status: "all",
    date: "all",
...
expect(
  filterAppointments(appointments, customers, {
    ...
    status: "all",
    date: "2026-07-10",
  }).every((item) => item.startsAt.startsWith("2026-07-10")),
).toBe(true);
expect(
  validateAppointment(
...
describe("DashboardAvailability", () => {
  ...
  it("updates both simulated reminder channels", () => {
    ...
    expect(
      screen.getByText("يتم إرسال التذكير قبل الموعد بـ 24 ساعة في النسخة النهائية."),
    ).toBeTruthy();
  });
});
```
